...
export async function deleteBulkJobApplications(ids: string[]): Promise<void> {
    ...
    // Fetch the applications to find its attached files
    const { data: applications, error: appError } = await supabase
        .from('job_applications')
        .select('id, resume_file_path, cover_letter_file_path')
        .in('id', ids);
    ...
    if (filesToDelete.length > 0) {
        ...
        if (storageError) {
            console.error('Failed to delete application files from bucket in bulk:', storageError);
        }
    }
    ...
    if (deleteError) {
        throw new Error(`Failed to delete bulk applications: ${deleteError.message}`);
    }
    ...
    await logActivity({
        ...
        resourceType: 'job_application',
        resourceId: 'bulk',
        resourceTitle: `Deleted ${ids.length} job applications`
    });
